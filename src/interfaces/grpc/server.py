...
class DBCServicer:
    ...
    async def ProcessFrames(self, request_iterator: AsyncIterator, context) -> AsyncIterator:
        async for request in request_iterator:
            if self._message_handler:
                asyncio.create_task(self._handle_frame(request.topic, request.payload))
    ...
    def _create_response(self, message: ParsedMessage):
        return type(
            "Response",
            (),
            {
